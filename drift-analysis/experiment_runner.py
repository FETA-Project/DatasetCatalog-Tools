...
if DATE:
    ...
    experiment_config = ExperimentConfig(
        ...
        index_column = "date", # Action: Change to the name of the index column
        window_length = timedelta(days=1),
        global_test = WassersteinTest(drift_threshold_global=0.04,drift_threshold_single = 0.1), 
        experiment_name = "Baseline",
        target_column = "APP", # Action: Ghange to the name of the label column
    ...
    experiment_config = ExperimentConfig(
        data = df,
        chosen_features = feat_names,
        index_column = "index", # Action: Change to the name of the index column
        window_length = 10000,
        global_test = WassersteinTest(drift_threshold_global=0.04,drift_threshold_single = 0.1), 
        experiment_name = "Baseline",
        target_column = "label", # Action: Ghange to the name of the label column
        class_test = KSTest(drift_threshold_global=0.475,drift_threshold_single = 0.05), 
        analyser_test = None,
