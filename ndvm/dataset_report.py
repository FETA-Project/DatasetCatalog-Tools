...
class dataset_metrics:
    ...
    def eval_metrics(self, dataset, label, output_dir_metadata_base):
        ...
        if self.cfg["remove"]:
            df_dataset = df_dataset.drop(columns=self.cfg["remove"])
    ...
    def get_katoda_report(self):
        ...
        """
        report = {}
        insights = " The dataset has {} small classes, {} duplicated samples, {} nan value".format(self.filtered_label,self.duplicated, self.nan)
        ...
        try:
            ...
            self.scores["Association"] = {"Max Clf Score": "","Association": "", "P-value status":"", "":""}
        try:
            self.scores["Similarity"]["metric"]
        except Exception as e:
            self.scores["Similarity"] = {"metric":""}
        try:
            self.scores["Redundancy"]
        except Exception as e:
            self.scores["Redundancy"] = ""
