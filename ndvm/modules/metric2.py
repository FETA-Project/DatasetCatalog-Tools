...
class Association(AbstractMetric):
                ...
                for i in range(self.nperm):
                    for j in range(len(self.perc)):
                        ...
                        while True:
                            ind_pool = {}
                            nperc_pool = {}
                            for item in self.y1.value_counts().items():  
                                ind_pool[item[0]] = np.where(self.y1 == item[0])
                                nperc_pool[item[0]] = round(self.perc[j] * len(ind_pool[item[0]][0]) / 100)
                                
                            tmp_list = []
                            for item in self.y1.value_counts().items():    
                                tmp_list.append(ind_pool[item[0]][0][:nperc_pool[item[0]]])
                            ...
                            y1P = np.array(self.y1).copy()
                            y1P[ind] = np.array(self.y1[indP]).copy()
    ...
    def save_results(self,outputdir):
        ...
        """
        # Clean orginal dataset from export
        del self.datasets
        del self.X1 
        del self.y1 
